...
export default {
  ...
  data () {
    return {
      ...
      maskIdx: 0, // 蒙板的layerIdx (单/双)
      maskIdx_R: 0, // 左边蒙板的layerIdx
      isCutImg: false, // 是否裁剪
  ...
  methods: {
    sortObjs() {
      ...
      this.canvas.forEachObject((item)=> {
        ...
        if(item.name) {
          ...
          unHasIdxArr.push(item) // type: "polygon"
        }*/
      })
      hasIdxArr.sort((a, b)=> {
        return a.layerIdx - b.layerIdx
      })
      // Array.prototype.push.apply(hasIdxArr, unHasIdxArr)
      ...
      this.$nextTick(function() {
        if(this.isSingle) {
          if(newVal) {
            ...
            this.maskIdx = newVal.layer.length
            ...
            var data = newVal.layer.map((item, idx)=> {
              ...
              if(item.layertype == 1) {
                ...
                if(item.img.mask) {
                  this.maxLayerIdx++ // 有蒙版 maxLayerIdx++
                  this.maskIdx++
                }
                if(item.img.fillImg) {
                  this.fillImgNum++
          ...
          if(newVal) {
            ...
            this.maskIdx = newVal[0].layer.length
            ...
            this.maskIdx_R = newVal[1].layer.length
            var data2 = newVal[1].layer.map((item)=> {
              ...
              if(item.layertype == 1) {
                ...
                item.picCount = newVal[1].photocount // 该页的蒙版数量
                if(item.img.mask) {
                  this.maxLayerIdx_R++ // 有蒙版 maxLayerIdx_R++
                  this.maskIdx_R++
                }
                if(item.img.fillImg) {
                  this.fillImgNum++
    ...
    toPathFn(src, l, t, canvas, clipPathData, layerIdx) {
      ...
      img1.onload = ()=> {
        ...
        // fabric.Polygon.prototype.layerIdx = layerIdx
