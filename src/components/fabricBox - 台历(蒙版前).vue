...
export default {
  name: 'fabricBox',
  props: ["idName", "pageData", "curDragImg", "lastSave", "isEditImg", "dragType", "fontFamily"],
  data () {
    return {
      canvas: '',
      cPath: '',
    ...
    pageData(newVal, oldVal) {
      ...
      if(newVal) {
        this.$nextTick(function() {
          ...
          this.canvas.setHeight(460)
          this.cPath.setWidth(800)
          this.cPath.setHeight(460)
          this.canvas.clear()
          if(newVal.oldFabric) {
  ...
  methods: {
    ...
    addLayer(newVal, canvas, clipPathData) {
      var vm = this
      // 0底层背景,1需要填充图片,2照片框,3前景图,5可编辑文字
      this.time1 = setTimeout(()=>{
        if(newVal.layer) {
          newVal.layer.forEach((item)=> {
            if(item.layertype == 3) { // 背景
              // setOverlayImage
              // canvas.setBackgroundImage("http://pex7ss6vd.bkt.clouddn.com/" + item.img.url, canvas.renderAll.bind(canvas));
              ...
              vm.addBg("http://pex7ss6vd.bkt.clouddn.com/" + item.img.url, canvas, { 
                name: 'bgImg',
                id: item.img.id,
                left: l,
                top: t,
      ...
      this.time2 = setTimeout(()=>{
        if(newVal.layer) {
          newVal.layer.forEach((item)=> {
            if(item.layertype == 1) { // 1需要填充图片
              /*var rect = item.rect.split(';')
              var l = rect[0]*1
              var t = rect[1]*1*/
              var l = item.primitiveX * this.deskScale
              var t = item.primitiveY * this.deskScale
              vm.addBg("http://pex7ss6vd.bkt.clouddn.com/" + item.img.mask.url, canvas, { 
                name: 'clipBox',
                id: item.img.mask.id,
                left: l,
                top: t,
              ...
              vm.toPathFn("http://pex7ss6vd.bkt.clouddn.com/" + item.img.mask.url, l, t, canvas, clipPathData)
            }
          })
        }
      }, 200)
      ...
      if(this.dragType == '照片') {
        vm.clipPathData.forEach((item)=> {
          ...
          if(isIn) {
            ...
            if(this.curDragImg) {
              console.log(this.curDragImg.src)
  ...
  mounted() {
    this.$nextTick(function() {
      ...
      this.canvas.on('mouse:down', (options)=> {
        ...
        var y = e.offsetY
        var gobj = this.canvas.getActiveObject(); //获取当前选中对象
        if(gobj && (gobj.name == "text" || gobj.type == 'text')) {
          this.$emit('getObj', gobj)
