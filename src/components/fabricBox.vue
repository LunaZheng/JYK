<template>
  <div class="fabricBox" @dragover="allowDrop" @drop="drop" style="position: relative;">
    ...
    <!-- 图片裁剪 --> 
    <imgCut v-if="isCutImg" :curCutImg="gobj" :isCutImg="isCutImg" @getIsCutImg="getIsCutImg" @clip-save="getClipSave"></imgCut>
    <!-- 图片编辑框 --><!-- 装饰编辑框 -->
    ...
    <fontEdit v-if="isEditFont" :fontData="fontData" :curEditFont="gobj" :cStep="cStep" :paga2AddL="paga2AddL" @updateModifications="updateModifications" :deskScale="deskScale" @close="close"></fontEdit>
  </div>
</template>
...
export default {
  name: 'fabricBox',
  props: ["idName", "isSingle", "deskScale", "tempURL", "pageData", "curDragDecorate", "filePicData", "dragData", "fontData"],
  components: {
    imgEdit,
    fontEdit,
    imgCut
  ...
  data () {
    return {
      ...
      clipPathData: [], // 当前画布 clipPath 对象集合
    ...
    'canvas._objects': function(newVal, oldVal) {
      if(newVal.length > 0) {
        if(!this.isInitOver && newVal.length == this.inItOverObjNum) { // 画布初始
          ...
          // 检测 已经填充 / 无需填充
          if(this.isWatchFilled && this.isInitOver) {
  ...
  methods: {
    sortObjs() {
      ...
      this.canvas.forEachObject((item)=> {
        // console.log(item.name, item.layerIdx)
        item.hasControls = false // 取消角控件
        if(item.name == 'text' || item.name == 'oText' || item.name == 'clipPath' || item.name == 'decorate' || item.name == 'textBox') this.canvas.bringToFront(item).renderAll() //置顶
        if(item.name != 'clipPath') {
          hasIdxArr.push(item)
        } else if(item.name == 'clipPath') {
          clipArr.push(item)
        }
      })
      hasIdxArr.sort((a, b)=> {
      ...
      Array.prototype.push.apply(hasIdxArr, clipArr)
      ...
      if(this.isWatchFilled && this.isInitOver) {
        ...
        // console.log(fillPicNum, this.wantPicNum)
        if(fillPicNum == this.wantPicNum) {
          setTimeout(()=> {
            this.$emit('getIsFilled', true)
          }, 200);
      ...
      this.$nextTick(()=> {
        if(this.isSingle) { // 单页
          if(newVal) {
            ...
            let ctxPath = document.getElementById('cPath_' + this.idName);
            ctxPath.width = this.cPath.width
            ctxPath.height = this.cPath.height
            this.canvas.clear()
            this.cPath.clear()
            var bgNum = 0
          ...
          if(newVal) {
            ...
            // console.log(Math.ceil(pW * this.deskScale), Math.ceil(pH * this.deskScale))
            this.canvas.clear()
            this.cPath.clear()
            // 设置画布宽高
            ...
            /* setWidth 和 setHeight 等同于 设置了style的宽高属性 导致 拉伸
               解决: ① <canvas width="500" height="500" ></canvas>
                     ②使用HTML5 Canvas API操作 (如下)
            */
            let ctxPath = document.getElementById('cPath_' + this.idName);
            ctxPath.width = this.cPath.width
            ctxPath.height = this.cPath.height
            ...
            var data2 = newVal[1].layer.map((item)=> {
              ...
              this.paga2AddL = newVal[0].pagewidth * this.deskScale // 第二页要加的宽度
    ...
    toPathFn(src, l, t, layerIdx) {
      ...
      img1.onload = ()=> {
        ...
        imgData = this.ctxPath.getImageData(l, t, this.cPath.width, this.cPath.height);
        ...
        this.cPath.remove(namedImg)
        clipPath.set({
          left: clipPath.left * 1 - 1,
          top: clipPath.top * 1 - 1,
        })
        this.clipPathData.push({
          obj: clipPath,
          src: src
        })
          ...
          if(item.layertype == 1) { // 1 底层背景
            ...
            if(item.img.url) {
              ...
              if(item.img.url.indexOf('bfl-img') > -1) {
                ...
                url = this.tempURL + item.img.url + '?imageView2/2/w/' + w.toFixed(0) + '/h/' + h.toFixed(0) +  '/format/png'
              }
              this.addImg(url, { 
                name: 'bgImg',
                id: item.img.id,
            ...
            if(item.img.mask) {
              ...
              var w = item.primitiveWidth * this.deskScale
              var h = item.primitiveHeight * this.deskScale
              this.addImg(this.tempURL + item.img.mask.url + '?imageView2/2/w/' + w.toFixed(0) + '/h/' + h.toFixed(0) +  '/format/png', { 
                name: 'clipBox',
                id: item.img.mask.id,
                layerIdx: item.idx - 1,
                left: l,
              ...
              this.toPathFn(this.tempURL + item.img.mask.url + '?imageView2/2/w/' + w.toFixed(0) + '/h/' + h.toFixed(0) +  '/format/png', l, t, item.idx)
            }
            if(item.img.fillImg) {
              ...
              // console.log(item.img.fillImg.url)
              this.addImg(item.img.fillImg.url, {
                ...
                perPixelTargetFind: true, // 是否能拖动空的部分(默认false)
              }, item.img.fillImg.scale, false)
              // this.addFillImg(item.img.fillImg.clipObj, item.img.fillImg.url, false)
            }
    ...
    addImg(src, attrs, scale, isAddPic, changeBg) {
      // isAddPic(拖拽添加fillImg) changeBg(改变背景图片)
      var vm = this
      var img = new Image()
      img.crossOrigin = '';
      // new fabric.NamedImage.fromURL(src, (namedImg)=> {})
      img.onload = function() { // 加载完成 
        ...
        if(namedImg.name == 'bgImg') {
          ...
          namedImg.set('perPixelTargetFind', true) // 是否能拖动空的部分(默认false)
          vm.canvas.renderAll()
          // console.log(vm.isInitOver, isAddPic)
          vm.isTempDataLoading = false
          if(vm.isInitOver && isAddPic) {
            vm.sortObjs()
            vm.updateModifications() // 添加画布记录
          }
        } else if(namedImg.name == 'decorate') {
          if(vm.isInitOver) vm.updateModifications() // 添加画布记录
        }
      }
      img.src = src // 要放在img.crossOrigin = '';之后, 否则会报跨域的错
    },
    ...
    drop(e) {
      ...
      if(!dragData) dragData = this.dragData
      this.canvas.discardActiveObject() // 不选中
      this.gobj = ''
      this.isEditImg = false
      this.isEditFont = false
    ...
    addFillImg(clipObj, picSrc, alertTip) {
      // console.log(JSON.parse(JSON.stringify(clipObj)))
      this.isTempDataLoading = true
      var l = clipObj.left
      var t = clipObj.top
      var w = clipObj.width
      ...
      if(picSrc) {
        this.$axios.get(picSrc.split('?')[0] +  '?imageInfo').then(response => {
      ...
      objs.forEach((item, itemIdx)=> {
        ...
        if(item.name == 'bgImg') {
          ...
          for(let i = 0, len = oldPageData.layer.length; i < len; i++) {
            // console.log(oldPageData.layer[i], oldPageData.layer[i].idx, item.layerIdx)
            if(oldPageData.layer[i].idx == item.layerIdx) { // push 入蒙版|背景
              // console.log(oldPageData.layer[i], item)
          ...
          var w = item.width / this.deskScale
          var h = item.height / this.deskScale
          var src = item.src.replace(this.tempURL, '').split('?')[0]
          var obj = {
            layertype: 2,
            primitiveX: x,
            primitiveY: y,
