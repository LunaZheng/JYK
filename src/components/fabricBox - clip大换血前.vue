<template>
  <div class="fabricBox" @dragover="allowDrop" @drop="drop" style="position: relative;">
    ...
    <!-- 图片编辑框 --><!-- 装饰编辑框 -->
    <imgEdit v-if="isEditImg" :curEditImg="gobj" :cStep="cStep" @updateModifications="updateModifications" @close="close" @getIsCutImg="getIsCutImg"></imgEdit>
    <!-- 文字编辑框 v-show(才能触发word编辑框消失改变) -->
    <fontEdit v-show="isEditFont" :curEditFont="gobj" :cStep="cStep" @updateModifications="updateModifications" :deskScale="deskScale" @close="close"></fontEdit>
  </div>
</template>
...
export default {
  name: 'fabricBox',
  props: ["idName", "isSingle", "deskScale", "pageData", "curDragImg", "dragType", "fontFamily", "curDragDecorate"],
  components: {
    imgEdit,
    fontEdit,
    imgCut
  ...
  watch: {
    ...
    'canvas._objects': function(newVal, oldVal) {
      if(newVal) {
        ...
        if(newVal.length == this.inItOverObjNum) { // 画布初始
  ...
  methods: {
      ...
      this.$nextTick(function() {
        if(this.isSingle) {
          ...
          if(newVal) {
            ...
            this.addLayer(newVal[0], this.canvas, this.clipPathData)
    ...
    toPathFn(src, l, t, canvas, clipPathData, layerIdx, mask) {
      ...
      img1.onload = ()=> {
        ...
        mask.maskPathData = JSON.parse(JSON.stringify(obj))
        ...
        newVal.layer.forEach((item)=> {
          ...
          if(item.layertype == 1) { // 1 底层背景
            ...
            if(item.img.mask) {
              ...
              this.toPathFn("http://img.res.excelll.org/" + item.img.mask.url + '?imageView2/2/w/' + w.toFixed(0) + '/h/' + h.toFixed(0) +  '/format/png', l, t, canvas, clipPathData, layerIdx, item.img.mask)
              console.log(item.img.mask)
            }
            if(item.img.fillImg) {
    ...
    addImg(src, attrs, scale, picCount) {
      var vm = this
      var img = new Image()
      img.src = src
      img.crossOrigin = '';
    ...
    addFillImg(clipObj, picObj, alertTip) {
      var l = clipObj.clipPath.left
      var t = clipObj.clipPath.top
      var w = clipObj.clipPath.width
      var h = clipObj.clipPath.height
      if(picObj) {
        this.$axios.get(picObj.src.split('?')[0] +  '?imageInfo')
          .then(response => {
          var pic = response.data
          var isWide = pic.width > pic.height
          if(isWide) { // 宽图
            var imgSrc = picObj.src.split('?')[0] +  '?imageView2/2/h/' + h
            if(pic.height < h) { // 照片小于框
              imgSrc = picObj.src.split('?')[0] +  '?imageMogr2/thumbnail/x' + h
              if(w / h > pic.width / pic.height) {
                imgSrc = picObj.src.split('?')[0] +  '?imageMogr2/thumbnail/' + w + 'x'
              }
              if(alertTip) {
              ...
              if(w / h > pic.width / pic.height) {
                imgSrc = picObj.src.split('?')[0] +  '?imageView2/2/w/' + w
              }
            }
          } else { // 长图
            var imgSrc = picObj.src.split('?')[0] +  '?imageView2/2/w/' + w
            if(pic.width < w) { // 照片小于框
              ...
              if(w / h < pic.width / pic.height) {
                imgSrc = picObj.src.split('?')[0] +  '?imageView2/2/h/' + h
              }
            }
          }
          this.addImg(imgSrc, {
            ...
            clipObj: clipObj.clipPath, // 自定义属性--> 裁剪框
            clipTo: 'function(ctx) {this.myClipTo(ctx)}',
          })
    ...
    onePageFill(data) {
      if(data.length <= 0) {
        ...
        if(this.clipPathData.length > 0) {
          var imgNum = 0
          this.clipPathData.forEach((item, idx)=> {
            var imgData = ''
            if(this.isSingle) {
              if(this.curEditData.layer[item.layerIdx].layertype == 3 && !this.curEditData.layer[item.layerIdx].img.fillImg) imgData = data[idx - imgNum]
            } else {
              if(item.clipPath.left <= this.paga2AddL) {
                this.curEditData[0].layer.forEach((layerItem, layerIdx)=> {
                  if(item.layerIdx == layerItem.idx) {
                    if(this.curEditData[0].layer[layerIdx].layertype == 3 && !this.curEditData[0].layer[layerIdx].img.fillImg) {
                      imgData = data[idx - imgNum]
                ...
                this.curEditData[1].layer.forEach((layerItem, layerIdx)=> {
                  if(item.layerIdx == layerItem.idx) {
                    if(this.curEditData[1].layer[layerIdx].layertype == 3 && !this.curEditData[1].layer[layerIdx].img.fillImg) {
                      imgData = data[idx - imgNum]
                      console.log(imgData, idx)
                    } else {
                      imgNum++
            ...
            if(imgData) {
              this.isTempDataLoading = true
              this.addFillImg(item, imgData, false)
            } else {
