<template>
  <div class="fabricBox" @dragover="allowDrop" @drop="drop" style="position: relative;">
    ...
    <!-- 图片裁剪 -->
    <imgCut v-if="isCutImg" :curCutImg="gobj" :maxLayerIdx="maxLayerIdx" @getIsCutImg="getIsCutImg" @clip-save="getClipSave"></imgCut>
    <!-- 图片编辑框 --><!-- 装饰编辑框 -->
    <imgEdit v-if="isEditImg" :curEditImg="gobj" :cStep="cStep" @updateModifications="updateModifications" @close="close" :maxLayerIdx="maxLayerIdx" @getIsCutImg="getIsCutImg"></imgEdit>
    <!-- 文字编辑框 v-show(才能触发word编辑框消失改变) -->
    <fontEdit v-show="isEditFont" :curEditFont="gobj" :cStep="cStep" :maxLayerIdx="maxLayerIdx" @updateModifications="updateModifications" :deskScale="deskScale" @close="close"></fontEdit>
  </div>
</template>
  ...
  methods: {
    ...
    addImg(src, attrs, scale, picCount, changeBg) {
      ...
      img.onload = function() { // 加载完成  
        ...
        if(namedImg.name == 'bgImg') {
          ...
          if(changeBg) {
            // console.log(changeBg, vm.isInitOver)
    ...
    drop(e) { // @drop: 在元素拖动到放置目标后触发
      ...
      if(this.dragType == '照片') {
        ...
        // if(this.isInitOver) this.updateModifications() // 添加画布记录
