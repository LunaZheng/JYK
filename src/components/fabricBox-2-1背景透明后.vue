...
export default {
  name: 'fabricBox',
  props: ["idName", "pageData", "curDragImg", "lastSave", "isEditImg", "dragType", "fontFamily", "deskScale"],
  data () {
    return {
      canvas: '',
      cPath: '',
  ...
  methods: {
        ...
        if(newVal.layer) {
          newVal.layer.forEach((item)=> {
            if(item.layertype == 3) {
              ...
              var l = rect[0]*1 * this.deskScale
              var t = rect[1]*1 * this.deskScale
    ...
    addImg(src, attrs) {
      ...
      img.onload = function() {
        ...
        // namedImg.hasBorders = false // 取消控件-边
        // namedImg.hasRotatingPoint = false // 取消旋转控件
        // namedImg.centeredRotation = true // 中心旋转
        // namedImg.centeredScaling = true // 中心放大
        // namedImg.setControlVisible('tl', false) // 取消左上角控件
        // namedImg.setControlVisible('bl', false)
        // namedImg.setControlVisible('tr', false)
        // namedImg.setControlVisible('br', false)
        ...
        // namedImg.setControlVisible('mtr', false)
        // namedImg.rotatingPointOffset = 5 // 旋转控件高度
        // namedImg.set({
        //   // borderColor: 'red',
        //   cornerColor: '#409eff',
        //   cornerSize: 10,
        ...
        /*vm.canvas.forEachObject(function(o){
      ...
      this.canvas.on('object:added', (options)=> {
        ...
        this.canvas.forEachObject((item)=> { // _objects.forEach
          if(item) {
            ...
            if(item.name == 'bgImg' || item.name == 'text') {
