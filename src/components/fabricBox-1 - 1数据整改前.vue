          ...
          setTimeout(()=>{
            if(item.layertype == 1) {
              ...
              if(item.img.mask.url) {
                ...
                this.toPathFn("http://pex7ss6vd.bkt.clouddn.com/" + item.img.mask.url + '?imageView2/2/w/' + (item.primitiveWidth * this.deskScale).toFixed(0) + '/h/' + (item.primitiveHeight * this.deskScale).toFixed(0) +  '/format/png', l, t, canvas, clipPathData)
              }
    ...
    drop(e) { // @drop: 在元素拖动到放置目标后触发
      ...
      if(this.dragType == '照片') {
        this.clipPathData.forEach((item)=> {
          ...
          if(isIn) {
            console.log(item)
            this.curClipPath = item
            var l = this.curClipPath.left
            var t = this.curClipPath.top
            var w = this.curClipPath.width
            var h = this.curClipPath.height
            if(this.curDragImg) {
              ...
              this.addImg(this.curDragImg.src, {
                ...
                top: t + h/2,
                // hasControls: false,
                clipObj: this.curClipPath, // 自定义属性--> 裁剪框
                clipTo: 'function(ctx) {this.myClipTo(ctx)}',
              })
            }
          }
      ...
      this.canvas.on('object:added', (options)=> {
        ...
        this.canvas.forEachObject((item)=> { // _objects.forEach
          if(item) {
            ...
            // if(item.name == 'text') {
            if(item.name == 'bgImg' || item.name == 'text' || item.name == 'decorate') {
