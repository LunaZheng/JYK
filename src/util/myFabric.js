...
export default {
  namedImage: function() {
    $fabric.fabric.NamedImage = $fabric.fabric.util.createClass(fabric.Image, { // 继承默认类
      ...
      toObject: function() { // 这是fabric中对象序列化的基石
        // if(this.name == 'fillImg') console.log(this.layerIdx, this.clipTo);
      ...
      myClipTo: function(ctx) {
        ...
        // console.log(this.clipObj)
        var isPolygon = this.clipObj.name == 'clipPath' || this.clipObj instanceof fabric.Polygon || this.clipObj.type == 'polygon' || this.clipObj.type == 'my-polygon'; 
    ...
    $fabric.fabric.NamedImage.hasBorders = false
  ...
  myText: function() {
    $fabric.fabric.MyText = $fabric.fabric.util.createClass(fabric.Text, { // 继承默认类
      type: 'my-text',
      initialize: function(element, options) {
        this.callSuper('initialize', element, options)
        options && this.set('name', options.name)
    ...
    // 这样 loadFromJSON 将知道在JSON解析期间调用哪个方法
    $fabric.fabric.MyText.fromObject = function(object, callback) {
      // console.log(object)
      $fabric.fabric.Object._fromObject('MyText', object, callback, 'text');
    };
  },
  myPolygon: function() {
    $fabric.fabric.MyPolygon = $fabric.fabric.util.createClass(fabric.Polygon, { // 继承默认类
      type: 'my-polygon',
      initialize: function(element, options) {
        this.callSuper('initialize', element, options)
        options && this.set('name', options.name)
      ...
      toObject: function() { // 这是fabric中对象序列化的基石
        return $fabric.fabric.util.object.extend(this.callSuper('toObject'), {
          name: this.name,
          layerIdx: this.layerIdx,
        })
      },
    ...
    // 这样 loadFromJSON 将知道在JSON解析期间调用哪个方法
    $fabric.fabric.MyPolygon.fromObject = function(object, callback) {
      return $fabric.fabric.Object._fromObject('MyPolygon', object, callback, 'points');
    };
  },
  myRect: function() {
    $fabric.fabric.MyRect = $fabric.fabric.util.createClass($fabric.fabric.Rect, { // 继承默认类
      type: 'my-rect',
      initialize: function(options) {
        this.callSuper('initialize', options)
        options && this.set('name', options.name)
        options && this.set('boxName', options.boxName)
      },
      toObject: function() { // 这是fabric中对象序列化的基石
        return fabric.util.object.extend(this.callSuper('toObject'), {
          name: this.name,
    ...
    // 这样 loadFromJSON 将知道在JSON解析期间调用哪个方法
    $fabric.fabric.MyRect.fromObject = function(object, callback) {
      return $fabric.fabric.Object._fromObject('MyRect', object, callback);
    };
  },
  myCircle: function() {
    // console.log($fabric.fabric.Circle)
    $fabric.fabric.MyCircle = $fabric.fabric.util.createClass(fabric.Circle, { // 继承默认类
      type: 'my-circle',
      initialize: function(element, options) {
        this.callSuper('initialize', element, options)
        options && this.set('name', options.name)
    ...
    // 这样 loadFromJSON 将知道在JSON解析期间调用哪个方法
    $fabric.fabric.MyCircle.fromObject = function(object, callback) {
      // console.log(object)
      $fabric.fabric.Object._fromObject('MyCircle', object, callback);
    };
  },
  myTriangle: function() {
    $fabric.fabric.MyTriangle = $fabric.fabric.util.createClass(fabric.Triangle, { // 继承默认类
      type: 'my-triangle',
      initialize: function(element, options) {
        this.callSuper('initialize', element, options)
        options && this.set('name', options.name)
    ...
    // 这样 loadFromJSON 将知道在JSON解析期间调用哪个方法
    $fabric.fabric.MyTriangle.fromObject = function(object, callback) {
      // console.log(object)
      $fabric.fabric.Object._fromObject('MyTriangle', object, callback);
    };
  },
  myPath: function() {
    $fabric.fabric.MyPath = $fabric.fabric.util.createClass(fabric.Path, { // 继承默认类
      type: 'my-path',
      initialize: function(element, options) {
        this.callSuper('initialize', element, options)
        options && this.set('name', options.name)
    ...
    // 这样 loadFromJSON 将知道在JSON解析期间调用哪个方法
    $fabric.fabric.MyPath.fromObject = function(object, callback) {
      // console.log(object)
      $fabric.fabric.Object._fromObject('MyPath', object, callback, 'path');
    };
  },
}
