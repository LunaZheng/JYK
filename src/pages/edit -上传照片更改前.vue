    ...
    <div class="edit-content">
      ...
      <div class="canvas-box">
        <div @dragover="allowDrop">
          <fabricBox1 idName="canvasPrev" :pageData="pagePrevData" :curDragImg="curDragImg" @getJson="getPagePrevData" :lastSave="lastSave" @getObj="getObj" :isEditImg="isEditImg" :dragType="dragType" :fontFamily="fontFamily" :deskScale="deskScale" v-if="isDesk"></fabricBox1>
          <fabricBox2 idName="canvasPrev" :pageData="pagePrevData" :curDragImg="curDragImg" @getJson="getPagePrevData" :lastSave="lastSave" @getObj="getObj" :isEditImg="isEditImg" :dragType="dragType" :fontFamily="fontFamily" :deskScale="deskScale" v-if="!isDesk"></fabricBox2><fabricBox2 idName="canvasNext" :pageData="pageNextData" :curDragImg="curDragImg" @getJson="getPageNextData" :lastSave="lastSave" @getObj="getObj" :isEditImg="isEditImg" :dragType="dragType" :fontFamily="fontFamily" :deskScale="deskScale" v-if="!isDesk"></fabricBox2>
        </div>
        ...
        <div class="main" v-if="curPicTab == 1">
          ...
          <ul class="picList" v-show="!isPicListLoading" @click="toggleCheck">
            <li v-for="(item, idx) in picListData">
              <div class="pic" :style="'background-image: url(' + item.imgpath + '?imageView2/2/w/105)'" :name="item.filename" :originSrc="item.imgpath" @dblclick="previewPic(item.imgpath)">
...
export default {
  ...
  methods: {
    ...
    updateOne(e) { // 开始上传(单个)
      var i = $(e.target).closest('tr').index()-1
      if(this.oFileArr.length > 0) {
        var curFileObj = this.oFileArr[i]
        this.update(curFileObj.oFile, curFileObj.code)
        this.oFileArr.splice(i, 1)
      }
    },
    updateAll() { // 全部开始上传
      // ...选择文件夹...
  ...
  mounted() { 
    ...
    // this.filePageData = JSON.parse(JSON.stringify(tempData1.tplInfos.page)) // 台历
    // 加上扉页
    ...
    if(this.isDesk) {
      // 台历数据测
      // 0底层背景,1需要填充图片框,2照片,3前景图,5可编辑文字
      /*this.pagePrevData = this.pageNextData = */
    } else {
      // 相册数据
...
.edit .edit-bottom .pic-box .img-list .ul1 li h1 {position: absolute; width: 90px; height: 20px; left: 50%; margin-left: -45px; line-height: 20px;}
...
.table-box table td .picName {display: inline-block;width: 200px; vertical-align: middle;}
