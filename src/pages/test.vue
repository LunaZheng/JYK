...
export default {
  name: 'test',
  created() {
    console.log(window.location.origin.indexOf('localhost'))
  ...
  methods: {
    toJSON() {
      this.page.layer.map((item2, idx2)=> {
        if(item2.img.url) {
          if(item2.img.url.indexOf('http') == -1) {
            item2.img.url = 'http://zhengrongrong.top' + item2.img.url
          }
        } else if(item2.mask.url) {
          if(item2.mask.url.indexOf('http') == -1) {
            item2.mask.url = 'http://zhengrongrong.top' + item2.mask.url
          }
        }
        return item2
      })
