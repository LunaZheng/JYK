        ...
        <ul :class="'edit-' + item.tag + '-list item-list'" v-if="idx === curShowListIdx" :style="{'top': 40 * (idx + 1) + 'px', 'padding-top': '2px'}" @click="stopBubble">
          <li v-for="(v, i) in item.item" :class="{'on': i === 0}">
            <img draggable="true"
              ...
              @dragstart="drag(item.name, i, $event)"
            >
            ...
            <img draggable="true"
              :src="'http://img.res.excelll.org/' + v.thumb + '?imageView2/2/h/150'" alt="" :title="i" 
              v-if="item.name === '模板'"
              @dragstart="drag(item.name, i, $event)"
            >
    ...
    <div class="edit-content">
      ...
      <div class="canvas-box">
        <div @dragover="allowDrop">
          <!-- <fabricBox v-show="isSingle" :curDragImg="curDragImg" :dragType="dragType" :fontFamily="fontFamily" :curDragDecorate="curDragDecorate"></fabricBox> -->
          <fabricBox v-if="isSingle" idName="canvasSingle" ref="fabricBox1" :isSingle="isSingle" :deskScale="deskScale" :pageData="pageJsonData" :curDragImg="curDragImg" :dragType="dragType" :fontFamily="fontFamily" :filePicData="filePicData" @getIsInitOver="getIsInitOver" @getSaveData="getSaveData" @hiddenPic="hiddenPic" @getIsFilled="getIsFilled" @changeTemp="changeTemp"></fabricBox>
...
export default {
  ...
  created() {
    this.$axios.get('http://222.73.182.225:8888/template.html?t=font').then(response => {
  ...
  watch: {
    ...
    isAllFill(newVal) { // 整本填充
      if(newVal) {
        if(this.isSingle) {
          this.$refs.fabricBox1.isWatchFilled = true
        } else {
          this.$refs.fabricBox2.isWatchFilled = true
        }
        ...
        if(this.isSingle) {
          this.$refs.fabricBox1.isWatchFilled = false
        } else {
          this.$refs.fabricBox2.isWatchFilled = false
        }
      }
    },
    isInitOver(newVal) {
  ...
  methods: {
    ...
    drag(name, idx, e) { // @dragstart: 在用户开始拖动元素时执行
      ...
      if(name != '照片') {
        ...
        switch (name) {
          case '模板': // 模板
            this.changeTempIdx = $(e.target).closest('li img').attr('title')
            break
          case '文字': // 文字
            // this.fontFamily = this.editList[1].item[idx].title.split(' ')[0]
            this.fontFamily = $(e.target).closest('.font-box').attr('title')
            break
          default: // 背景 图片 装饰 布局
    ...
    update(file, idx) { // 上传单个图片文件
      ...
      this.$axios.get('http://222.73.182.225:8888/qiniu.html?t=qiniu', this.qs.stringify(tokenParem)).then(response => {
    ...
    uploading(param, config, idx, updatePicsList) { // 七牛处理 & 上传图片到服务器
      ...
      this.$axios.post('http://upload.qiniu.com/', param, config).then(res1 => { // 上传图片到七牛
        if(res1.status == 200) {
          ...
          this.$axios.get('http://pcpwiayu4.bkt.clouddn.com/' + key + '?imageInfo').then(res2 => { // 从七牛获取图片基本信息
            if(res2.status == 200) {
    ...
    delPic(idx) {
      var pics = [].slice.call($('.ul1 li.pics'))
      for (let i = pics.length - 1; i >= 0; i--) { // 逆向循环 --> 一次性删除数组中的多个元素
        if ($(pics[i]).hasClass('on')) {
          this.filePicData.splice($(pics[i]).index(), 1);
        }
      }
    ...
    toSave() {
      ...
      if(fillImgNum == tempDataInfo.photocount) {
        ...
        this.$confirm('未完成编辑, 已保存到未完成作品中', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '查看',
          type: 'success'
        }).then(() => {
    ...
    onePageFill() {
      if(this.filePicData.length > 0) {
        let clipPathData = this.isSingle ? this.$refs.fabricBox1.clipPathData :  this.$refs.fabricBox2.clipPathData
        var picArr =  []
        this.filePicData.forEach((pic, idx)=> {
          ...
          if(pageMaskUrl.length > 0) {
            clipPathData.forEach((clip)=> {
              pageMaskUrl.forEach((url, idx)=> {
                if(clip.url.indexOf(url) > -1) {
                  if(this.isSingle) {
                    this.$refs.fabricBox1.addFillImg(clip, picArr[idx], false)
                  } else {
                    this.$refs.fabricBox2.addFillImg(clip, picArr[idx], false)
                  }
                }
              })
            })
  ...
  mounted() { 
    ...
    var obj2 = {
      src: 'http://pcpwiayu4.bkt.clouddn.com/FvACkvVCJzhP5AukQmmv9uymKTd_',
      name: 'seying.jpg' 
    }
    this.filePicData.push(obj2)
    var obj3 = {
      src: 'http://pcpwiayu4.bkt.clouddn.com/FmBogpun0Tx_jiOW-zqOy5XgfNSC',
      name: 'seying.jpg' 
    }
    ...
    this.filePicData.push(obj4)
    /*var obj5 = {
      src: 'http://pcpwiayu4.bkt.clouddn.com/FnnADUOMXNcdkqyRrDeccej8HlFr',
      name: 'seying.jpg' 
    }
    this.filePicData.push(obj5)
    var obj6 = {
      src: 'http://pcpwiayu4.bkt.clouddn.com/FvACkvVCJzhP5AukQmmv9uymKTd_',
      name: 'seying.jpg' 
    }
    this.filePicData.push(obj6)
    var obj7 = {
      src: 'http://pcpwiayu4.bkt.clouddn.com/FmBogpun0Tx_jiOW-zqOy5XgfNSC',
      name: 'seying.jpg' 
    }
    this.filePicData.push(obj7)
    var obj8= {
      src: 'http://pcpwiayu4.bkt.clouddn.com/Fj9kgD7FEdIEmCxN6d7S8Nv4XaIj',
      name: 'seying.jpg' 
    }
    this.filePicData.push(obj8)
    var obj9 = {
      src: 'http://pcpwiayu4.bkt.clouddn.com/FnnADUOMXNcdkqyRrDeccej8HlFr',
      name: 'seying.jpg' 
    }
    this.filePicData.push(obj9)
    var obj10 = {
      src: 'http://pcpwiayu4.bkt.clouddn.com/FvACkvVCJzhP5AukQmmv9uymKTd_',
      name: 'seying.jpg' 
    }
    this.filePicData.push(obj10)
    var obj11 = {
      src: 'http://pcpwiayu4.bkt.clouddn.com/FmBogpun0Tx_jiOW-zqOy5XgfNSC',
      name: 'seying.jpg' 
    }
    this.filePicData.push(obj11)*/
  }
}
</script>
