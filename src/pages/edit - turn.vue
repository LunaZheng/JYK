<template>
  <div class="edit">
    ...
    <div class="edit-content">
      ...
      <div class="canvas-box">
        <div @dragover="allowDrop">
          <fabricBox v-show="isSingle" idName="canvasSingle" ref="fabricBox1" :isSingle="isSingle" :deskScale="deskScale" :pageData="pageJsonData" :curDragImg="curDragImg" :dragType="dragType" :fontFamily="fontFamily" :curDragDecorate="curDragDecorate" @getOldPageData="getOldPageData" @getIsInitOver="getIsInitOver"></fabricBox>
    ...
    <!-- 预览 -->
    <el-dialog class="dialogCSS" :visible.sync="isShowView">
      <div class="viewBox">
        <div id="canvas">
...
export default {
  ...
  data () {
    return {
      ...
      deskScale: 0,
      loadCanvas: '', // 撤销恢复后的数据
      isInitOver: false, // fabric 初始加载完成
      isShowView: true, // 预览
    }
  },
  watch: {
  ...
  methods: {
    ...
    getUploadImgList() {
      ...
      this.$axios.post(this.sApi.userphoto + '?t=search', params, {
        headers: {
          "Content-type": "application/x-www-form-urlencoded; charset=utf-8"
        }
      }).then(res => {
    ...
    delOnePic(upid) { // 删除单张照片
      this.$axios.get(this.sApi.userphoto + '?t=del&userid=' + JSON.parse(this.getCookie('userInfo')).vipno + '&upid=' + upid).then(res => {
        // console.log(res);
        if(res.data.type == 1) {
  ...
  mounted() { 
      ...
      flipbook.turn({
        ...
        when: {// Events
          turning: function(event, page, view) {
            ...
            disableControls(page); // 上/下一页按钮
          },
          turned: function(event, page, view) {
            disableControls(page);
            $(event.target).turn('center');
            if (page == 1) {
              $(event.target).turn('peel', 'br'); // 页脚翻起
            }
          },
          missing: function(event, pages) {
            // Add pages that aren't in the magazine
            for (var i = 0; i < pages.length; i++)
              addPage(pages[i], $(event.target));
          }
        }
      });
    ...
    $('#canvas').hide();
    // Load the HTML4 version if there's not CSS transform (兼容transform)
    yepnope({
      test: Modernizr.csstransforms,
      yep: ['/static/bfl-js/turn/turn.js'],
      nope: ['/static/bfl-js/turn/turn.html4.min.js'],
      both: ['/static/bfl-css/turn/magazine.css'],
      complete: loadApp
    });
    ...
    function loadPage(page, pageElement) {
      ...
      img.load(function() {
        ...
        $(this).appendTo(pageElement);// Add the image to the page after loaded
        pageElement.find('.loader').remove();// Remove the loader indicator
      });

      // Load the page
      // img.attr('src', 'pages/' + page + '.jpg');
      img.attr('src', 'http://img.res.excelll.org/' + filePageData[page-1].thumb + '?imageView2/2/w/105');
    }
