    ...
    <div class="edit-content">
      ...
      <div class="canvas-box">
        <div @dragover="allowDrop">
          <fabricBox1 v-show="isDesk" idName="canvasSingle" ref="fabricBox1" :pageData="pageDataSingle" :curDragImg="curDragImg" :dragType="dragType" :fontFamily="fontFamily" :curDragDecorate="curDragDecorate" @getOldPageData="getOldPageData"></fabricBox1>
          <div class="fabricBox2-box">
            <fabricBox2 v-show="!isDesk" idName="canvasPrev" ref="fabricBoxPrev" :pageData="pagePrevData" :curDragImg="curDragImg" :dragType="dragType" :fontFamily="fontFamily" :curDragDecorate="curDragDecorate" :deskScale="deskScale" :fStep="cStep" :fState="state" :loadCanvas="loadCanvas" @getStep="getStep" @getState="getState" @getJson="getPagePrevData"></fabricBox2><fabricBox2 v-show="!isDesk" idName="canvasNext" ref="fabricBoxNext" :pageData="pageNextData" :curDragImg="curDragImg" :dragType="dragType" :fontFamily="fontFamily" :curDragDecorate="curDragDecorate" :deskScale="deskScale" :fStep="cStep" :fState="state" :loadCanvas="loadCanvas" @getStep="getStep" @getState="getState" @getJson="getPageNextData"></fabricBox2>
          </div>
        </div>
      </div>
  ...
  watch: {
    curPages(newVal, oldVal) {
      ...
      if(this.isDesk) { // 单页编辑
        ...
        this.pageNextData = this.filePageData[newVal * 1 + 1]
      }
    },
    ...
    cStep(newVal, oldVal) {
      if(!this.isDesk) {
        if((newVal - 1) <= 0) {
          ...
          // this.oldCnavasObjs = JSON.stringify(this.canvas._objects)
          // console.log('2---' + this.oldCnavasObjs)
          $('li.undo a').css({
            'color': '#333',
            'cursor': 'pointer'
          }).hover(function(e) {
            $(e.target).css('color', '#3aaa92')
          }, function(e) {
  ...
  methods: {
    ...
    undo() { // 撤销
      if(this.isDesk) {
        ...
        if(this.cStep > 1) {
          this.cStep--
          this.loadCanvas = this.state[this.cStep].data
          /*console.log('')
          console.log(this.cStep, this.state)*/
          if(this.cStep == 1) {
            if(this.state[this.cStep].name == "canvasPrev") {
              this.$refs.fabricBoxPrev.undo(this.loadCanvas, this.cStep)
            } else if(this.state[this.cStep - 1].name == "canvasPrev") {
              this.$refs.fabricBoxPrev.undo(this.state[this.cStep - 1].data, this.cStep)
            } else if(this.state[this.cStep].name == "canvasNext") {
              this.$refs.fabricBoxNext.undo(this.loadCanvas, this.cStep)
            } else if(this.state[this.cStep - 1].name == "canvasNext") {
              this.$refs.fabricBoxNext.undo(this.state[this.cStep - 1].data, this.cStep)
            }
          } else {
            if(this.state[this.cStep].name == "canvasPrev") {
              this.$refs.fabricBoxPrev.undo(this.loadCanvas, this.cStep)
    ...
    redo() { // 恢复
      if(this.isDesk) {
        ...
        if(this.cStep < this.state.length-1) {
          ...
          /*if(this.cStep == 1) {
            if(this.state[this.cStep].name == "canvasPrev") {
              this.$refs.fabricBoxPrev.redo(this.loadCanvas, this.cStep)
            } else if(this.state[this.cStep + 1].name == "canvasPrev") {
              this.$refs.fabricBoxPrev.redo(this.state[this.cStep + 1].data, this.cStep)
            } else if(this.state[this.cStep].name == "canvasNext") {
              this.$refs.fabricBoxNext.redo(this.loadCanvas, this.cStep)
            } else if(this.state[this.cStep + 1].name == "canvasNext") {
              this.$refs.fabricBoxNext.redo(this.state[this.cStep + 1].data, this.cStep)
            }
          } else {
          }*/
          if(this.state[this.cStep].name == "canvasPrev") {
